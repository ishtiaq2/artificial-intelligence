...
## Teacher Script
Reasoning (Module 5) is thinking — Acting is doing something about it. This module covers two forms
...
AI is like that friend actually driving the car. Giving good directions (Module 5) and actually
steering the wheel (Module 6) are two different skills — a real agent needs both."
...
Two parts:
1. A tiny **tool-using agent** — an AI-style function that decides when to call a `calculator` tool
   vs. answer directly, illustrating the reasoning-then-acting pattern.
2. A **virtual self-driving decision loop** — a simplified sense-think-act simulation for a car.
